...
const DOC_GROUPS = [
  { label: "Letters of Support", ids: ["d1", "d2", "d3a", "d3b", "d3c", "d3d"] },
  ...
  return (
    <div className="space-y-6 fade-up">
        ...
        {(Object.keys(DOC_STATUS_CONFIG) as DocStatus[]).map((s) => {
          ...
          return (
            ...
            >
              {STATUS_ICONS[s]}
              <div>
                <p className="text-xl font-bold tabular-nums" style={{ color: cfg.dot }}>{summary[s]}</p>
      ...
      {DOC_GROUPS.map((group) => {
        ...
        return (
          <div key={group.label} className="bg-card rounded-lg border border-border shadow-sm overflow-hidden">
            {/* Group Header */}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/20">
                  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider w-10">#</th>
                  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider">Document</th>
                  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider hidden sm:table-cell w-16">Ref</th>
                  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider hidden md:table-cell w-36">Responsible</th>
                  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider w-36">Status</th>
                  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider hidden lg:table-cell">Notes</th>
                  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider w-20">Due</th>
                </tr>
              </thead>
              <tbody>
                {groupDocs.map((doc, i) => (
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <span className="font-mono text-xs font-bold text-muted-foreground">{doc.docNum}</span>
                    </td>
                    <td className="px-4 py-2.5">
                      <div className="flex items-start gap-2">
                        <span className="mt-0.5 flex-shrink-0">{STATUS_ICONS[doc.status]}</span>
                        <span className="text-xs font-medium leading-relaxed">{doc.title}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2.5 hidden sm:table-cell">
                      <span className="font-mono text-xs font-bold text-primary">{doc.sectionRef}</span>
                    </td>
                    <td className="px-4 py-2.5 hidden md:table-cell">
                      <TeamMemberSelect
                        selectedIds={[doc.responsibleId]}
                        onChange={(ids) => updateDocResponsible(doc.id, ids[0] ?? doc.responsibleId)}
                        multiple={false}
                      />
                    </td>
                    <td className="px-4 py-2.5">
                    ...
                    <td className="px-4 py-2.5 hidden lg:table-cell">
                      <input
                        ...
                        onChange={(e) => updateDocStatusNote(doc.id, e.target.value)}
                    ...
                    <td className="px-4 py-2.5">
                      <span className="text-xs font-semibold text-destructive whitespace-nowrap">{doc.dueDate}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
