...
export default function TeamTab() {
  ...
  const memberStats = useMemo(() => {
    return TEAM_MEMBERS.map((member) => {
      ...
      return { member, ownedSections, ownedDocs, ownedTasks, completedTasks, avgProgress };
    });
      ...
      {orgGroups.map(({ org, cfg, members }) => (
        <div key={org}>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider" style={{ color: cfg.textColor }}>
              {cfg.label}
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {members.map(({ member, ownedSections, ownedDocs, ownedTasks, completedTasks, avgProgress }) => (
              <div
                key={member.id}
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold truncate" style={{ color: cfg.textColor }}>{member.name}</p>
                    <p className="text-xs text-muted-foreground truncate">{member.role}</p>
                  </div>
                  <ProgressRing value={avgProgress} size={40} strokeWidth={3.5} color={cfg.color} />
                </div>

                {/* Stats Row */}
                <div className="grid grid-cols-3 divide-x divide-border border-b border-border">
                  <div className="px-3 py-2 text-center">
                    <p className="text-lg font-bold tabular-nums" style={{ color: cfg.color }}>{ownedSections.length}</p>
                    <p className="text-xs text-muted-foreground">Sections</p>
                  </div>
                  <div className="px-3 py-2 text-center">
                    <p className="text-lg font-bold tabular-nums" style={{ color: cfg.color }}>{ownedDocs.length}</p>
                    <p className="text-xs text-muted-foreground">Docs</p>
                  </div>
                  <div className="px-3 py-2 text-center">
                    <p className="text-lg font-bold tabular-nums" style={{ color: cfg.color }}>
                ...
                {ownedSections.length > 0 && (
                  <div className="px-4 py-3">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Assigned Sections</p>
                    <div className="space-y-1.5">
                      {ownedSections.slice(0, 5).map((s) => {
                        const stCfg = STATUS_CONFIG[s.status];
                        return (
                          <div key={s.id} className="flex items-center gap-2">
                            <span className="font-mono text-xs font-bold w-7 flex-shrink-0" style={{ color: cfg.color }}>{s.num}</span>
                      ...
                      {ownedSections.length > 5 && (
                        <p className="text-xs text-muted-foreground">+{ownedSections.length - 5} more sections</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                {ownedTasks.length > 0 && (
                  <div className="px-4 py-3 border-t border-border">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Timeline Tasks</p>
                    <div className="space-y-1">
                      {ownedTasks.slice(0, 3).map((task) => (
                        <div key={task.id} className="flex items-start gap-2">
                          <span
                            className="mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0"
                            style={{ background: task.done ? "#22C55E" : task.phaseColor }}
                          />
                      ...
                      {ownedTasks.length > 3 && (
                        <p className="text-xs text-muted-foreground">+{ownedTasks.length - 3} more tasks</p>
                      )}
                    </div>
                  </div>
                )}
